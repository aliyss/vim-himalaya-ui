...
function! s:list.view_as_html(view) abort
  ...
  let temp_file = tempname() . ".eml"

  let html = himalaya_ui#utils#request_plain_sync({
  \ 'cmd': 'message export --account %s --folder %s %s --full --destination %s',
  \ 'args': [shellescape(account), shellescape(folder), id, shellescape(temp_file)],
  ...
  " If g:himalaya_ui_eml_converter is mhonarc create a temporary file with the
  " content "test""
  let converter_args = g:himalaya_ui_eml_converter_args
  if g:himalaya_ui_eml_converter ==? 'mhonarc'
    let mrc_file = tempname() . ".mrc"
    " Add multiline variable to the file
    let content = [
        ...
        \  "</MIMEFILTERS>"
        \ ]
    call writefile(content, mrc_file)
    let converter_args = converter_args . ' -rcfile ' . shellescape(mrc_file)
  endif

  setlocal modifiable
  silent execute '%d'
  execute ('terminal cat ' . temp_file . ' | ' . g:himalaya_ui_eml_converter . ' ' . converter_args . ' | ' . g:himalaya_ui_html_viewer . ' ' . g:himalaya_ui_html_viewer_args)
  startinsert
